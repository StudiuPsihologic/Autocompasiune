...
class DayOneActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<WebView>(R.id.web_view_test).loadData(getString(R.string.day_one_text),
                "text/html; charset=utf-8", "utf-8")
    }

    fun finishDayOne(view: View) {
        ...
        with (sharedPref.edit()) {
            ...
            putInt("promise_text", R.string.promise_day_one)
            putInt("day_id", 2)
            apply()
        }

        setContentView(R.layout.activity_rest_day)
        val webView: WebView = findViewById<WebView>(R.id.rest_web_view)
        webView.setBackgroundResource(R.drawable.background)
        webView.setBackgroundColor(Color.TRANSPARENT)
        webView.loadData(getString(R.string.promise_day_one),"text/html; charset=utf-8", "utf-8")
    }
}